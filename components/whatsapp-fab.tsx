...
/** Opens WhatsApp with a short opener already typed. */
const WHATSAPP_URL = `${SITE.whatsappHref}?text=${encodeURIComponent("Hi, I'm interested!")}`;
...
export function WhatsAppFAB() {
  // The mobile menu's "Chat with us" button fires this event.
  useEffect(() => {
    window.addEventListener("open-whatsapp-chat", openWhatsAppDirect);
    return () => window.removeEventListener("open-whatsapp-chat", openWhatsAppDirect);
  }, []);
...
// ---------------------------------------------------------------------------
// Chat-form modal (name / suburb / products / home owner) — disabled.
// The button now opens WhatsApp directly. To restore, uncomment below,
// rename to WhatsAppFAB and add back: import { useState, useRef, useEffect, useCallback } from "react";
// ---------------------------------------------------------------------------
//
// const PRODUCTS = [
//   "Solar",
